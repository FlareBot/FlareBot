    ...
    public void onCommand(User sender, TextChannel channel, Message message, String[] args, Member member) {
        if(args.length >= 2){
            if(channel.getGuild().getSelfMember().hasPermission(channel, Permission.BAN_MEMBERS)){
                ...
                if(user == null){
                    channel.sendMessage(new EmbedBuilder().setDescription("We cannot find that user! Try their ID if you didn't already.").setColor(Color.red).build()).queue();
                    return;
                }
                //TODO: When reasons are out for JDA add them here!!!
                String reason = FlareBot.getMessage(args, 1);
                FlareBotManager.getInstance().getAutoModConfig(channel.getGuild().getId()).postToModLog(channel, user, sender, Punishment.EPunishment.BAN, reason);
