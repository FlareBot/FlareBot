...
public enum Action {
    ...
    SPAM(1, false, message -> FlareBot.getInstance().getAutoModTracker().getMessages(message.getGuild().getId(), message.getAuthor().getId())
    ...
    PROFANITY(1, true, message -> FlareBot.getInstance().getManager().getProfanity().stream().filter(word -> message.getContent().toLowerCase().contains(word)).count() > 0),
    ...
    TOO_MANY_CAPS(1, false, message -> message.getContent().replaceAll("[^a-zA-Z0-9]", "").length() > 4 && message.getContent().replaceAll("[^a-zA-Z0-9]", "").chars()
