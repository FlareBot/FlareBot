    ...
    public static boolean canRunCommand(CommandType type, User user) {
        ...
        if (type.isInternal()) {
            ...
            if (g.getMember(user) != null)
                return g.getMember(user).getRoles().contains(g.getRoleById(type.getRoleId())) || (FlareBot.instance().isTestBot() && g.getMember(user).getRoles().contains(Constants.getOfficialGuild().getRoleById(Constants.CONTRIBUTOR_ID)));
            else
                return false;
        } else
            return true;
