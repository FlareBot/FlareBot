...
import com.bwfcwalshy.flarebot.mod.Action;
        ...
        if(args.length == 1){
            if(args[0].equalsIgnoreCase("config")){
                ...
                EmbedBuilder builder = new EmbedBuilder().setColor(Color.white).addField("Auto Mod Enabled", String.valueOf(config.isEnabled()), true)
                        ...
                        .addField("Mod Log Channel", (config.getModLogChannel() != null && channel.getGuild().getTextChannelById(config.getModLogChannel()) != null ? channel.getGuild()
                                .getTextChannelById(config.getModLogChannel()).getAsMention() + " (" + config.getModLogChannel() + ")" : "Not Set"), true);
                StringBuilder sb = new StringBuilder();
                config.getPunishments().forEach((points, punishment) -> sb.append(points).append(" points = ").append(punishment.getName())
