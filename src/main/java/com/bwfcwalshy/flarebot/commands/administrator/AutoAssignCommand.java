            ...
            if (args.length == 0) {
                ...
                if (args[0].equalsIgnoreCase("list")) {
                    if (flareBot.getAutoAssignRoles().containsKey(channel.getGuild().getID())) {
                        ...
                        for(String role : flareBot.getAutoAssignRoles().get(channel.getGuild().getID())){
                            // This is there just incase they remove it.
                            if(getRole(channel.getGuild(), role) != null)
                                sb.append(getRole(channel.getGuild(), role).getName() + "\n");
                        }
                        sb.append("```");
